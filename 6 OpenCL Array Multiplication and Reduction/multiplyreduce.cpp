...
#ifndef NMB
#define NMB NUM_ELEMENTS / 1024 / 1024
#endif
...
const char *CL_FILE_NAME = {"multiplyreduce.cl"};
...
int main(int argc, char *argv[]) {
  ...
  status = clEnqueueWriteBuffer(cmdQueue, dA, CL_FALSE, 0, abSize, hA, 0, NULL,
                                NULL);
  if (status != CL_SUCCESS)
    fprintf(stderr, "clEnqueueWriteBuffer failed (1)\n");

  status = clEnqueueWriteBuffer(cmdQueue, dB, CL_FALSE, 0, abSize, hB, 0, NULL,
                                NULL);
  if (status != CL_SUCCESS)
  ...
  // NULL tells OpenCL that this is a local (shared) array, not a global array
  ...
  status =
      clEnqueueReadBuffer(cmdQueue, dC, CL_TRUE, 0, cSize, hC, 0, NULL, NULL);
  ...
  for (int i = 0; i < NUM_ELEMENTS; i++) {
    ...
    if (fabs(hC[i] - expected) > TOL) {
      // fprintf( stderr, "%4d: %13.6f * %13.6f wrongly produced %13.6f instead
      // of %13.6f (%13.8f)\n", i, hA[i], hB[i], hC[i], expected,
      // fabs(hC[i]-expected) );
      // fprintf( stderr, "%4d:    0x%08x *    0x%08x wrongly produced    0x%08x
      // instead of    0x%08x\n", i, LookAtTheBits(hA[i]), LookAtTheBits(hB[i]),
      // LookAtTheBits(hC[i]), LookAtTheBits(expected) );
    }
  }
