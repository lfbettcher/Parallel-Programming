...
# print header
printf "\nDGX,Num elements\n" >> arraymultsum.csv
printf "Local Work Size,," >> arraymultsum.csv
for n in "${num_elements[@]}"
do
        printf "%d," $n >> arraymultsum.csv
done
printf "\n" >> arraymultsum.csv
...
do
        printf ",%d," $s >> arraymultsum.csv
        # for each global array size
        for n in "${num_elements[@]}"
        do
                echo NUM_ELEMENTS = $n LOCAL_SIZE = $s
                g++ -o arraymultsum -DNUM_ELEMENTS=$n -DLOCAL_SIZE=$s first.cpp $libOpenCL -lm -fopenmp
                ./arraymultsum >> arraymultsum.csv
                echo ""
        done
        printf "\n" >> arraymultsum.csv
done
