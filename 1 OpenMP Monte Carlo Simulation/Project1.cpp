...
int main(int argc, char *argv[]) {
  ...
  omp_set_num_threads(NUMT);  // set the number of threads to use in
                              // parallelizing the for-loop:`

  // define these here so the rand() calls don't get into the thread timing:
...
#pragma omp parallel for default(none), shared(vs, ths, gs, hs, ds, stderr), reduction(+ : numHits)
  ...
  float probability = (float)numHits / (float)NUMTRIALS;  // last NUMTRIES run
  fprintf(stderr,
...
Print out:
    (1) number of threads
    (2) number of trials
    (3) probability of destroying the castle
    (4) MegaTrialsPerSecond
    Printing this as a single line with tabs or commas between the numbers is
    nice so that you can import these lines right into Excel.
*/

// Helper Functions: To choose a random number between two floats or two ints
