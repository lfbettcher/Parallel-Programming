...
MonteCarlo(float *dvs, float *dths, float *dgs, float *dhs, float *dds, int *dhits)
{
    ...
    if (x > g)
    {
        ...
        if (y > h)
        {
            ...
            if (disc < 0.)
            {
                printf("Discriminant should not be negative\n");
                return;
            }

            // ball hits ground above cliff
            ...
            // distance ball lands horizontally from the edge of the cliff
...
main(int argc, char* argv[])
{
    ...
    // "warm up" kernel, as suggested by TA Braxton because I was getting
    // worse results on DGX than rabbit and WSL.
    ...
    //printf("hits: %d trials: %d probability %3.2f%% megaTrials/sec %6.2lf,",
    //       totalHits, NUMTRIALS, 100. * probability, megaTrialsPerSecond);
    printf("%.2lf,", megaTrialsPerSecond);
